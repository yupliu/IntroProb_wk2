...
nycflights_avg_speed <- nycflights %>%
  mutate(avg_speed=distance*60.0/air_time) %>%
  select(avg_speed,tailnum) %>%
  arrange(desc(avg_speed))
nycflights_avg_speed
```
...
nycflights_speed_distance <- nycflights %>%
  mutate(avg_speed=distance*60.0/air_time)
ggplot(data=nycflights_speed_distance,aes(x=distance,y=avg_speed))+geom_point()
...
nycflights %>%
  ...
  summarise(ot_arr=sum(arr_type=="on time", dep_delay >0)/n())
